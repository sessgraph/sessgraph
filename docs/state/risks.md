...
| R-0011 | 打开 | Parent/Child Session 容易一次性引入复杂多 Agent 编排、capability delegation、真实 queue 或 reducer 框架，导致 core 范围失焦。 | ADR-0009 已定义 Parent/Child Session 边界；PR-0026 只允许实现 InMemory child session creation。child result dispatcher、reducer flow、capability delegation、真实 queue、database、server、provider 和 cloud 均后置单独立项。 |
