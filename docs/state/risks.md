...
| R-0010 | 打开 | Safety/Auth 后续实现容易过早接入 OAuth/OIDC、云 IAM、policy DSL、secrets manager 或业务 RBAC，导致 core 变成业务权限平台。 | ADR-0007 已定义 v0.5 边界；首个实现应只做 InMemory、FakeModel、deterministic tests 的 runtime-side policy gate，真实 identity provider 和 production policy 后续单独立项。 |
