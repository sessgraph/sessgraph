...
| R-0011 | 打开 | Parent/Child Session 容易一次性引入复杂多 Agent 编排、capability delegation、真实 queue 或 reducer 框架，导致 core 范围失焦。 | PR-0024 已决定下一阶段先做 Parent/Child Session ADR；ADR accepted 前不得实现 runtime，capability delegation、真实 queue、database、server、provider 和 cloud 均后置单独立项。 |
