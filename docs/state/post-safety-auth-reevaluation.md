...
2. PR-0026：InMemory child session creation flow，只覆盖 Child 创建和 relationship。已完成。
3. PR-0027：Child result Signal flow，只覆盖 terminal child result 回灌。
4. 后续再拆 reducer flow、Checkpoint recovery example/test。
5. capability delegation 等 Parent/Child Session 边界清楚后再单独 ADR。
6. 真实 identity provider、production policy、OAuth/OIDC、IAM、server、database 和 cloud deployment 继续后置到 integration 层，不进入下一阶段 core 主线。
...
下一步执行 PR-0027 / T-0029：基于 ADR-0009 和 PR-0026 实现 Child result Signal flow。该切片仍不实现 Parent reducer merge、capability delegation、真实 queue、database、server、provider 或 cloud。
