...
下一阶段优先进入 **Parent/Child Session**。PR-0025 已完成 ADR-0009；接下来进入 InMemory 本地确定性实现。

推荐顺序：

1. PR-0025：ADR 定义 Parent/Child Session 语义。已完成。
2. PR-0026：InMemory child session creation flow，只覆盖 Child 创建和 relationship，不实现 result dispatcher。
3. 后续再拆 child result Signal / reducer flow、Checkpoint recovery example/test。
4. capability delegation 等 Parent/Child Session 边界清楚后再单独 ADR。
5. 真实 identity provider、production policy、OAuth/OIDC、IAM、server、database 和 cloud deployment 继续后置到 integration 层，不进入下一阶段 core 主线。
...
## Parent/Child Session ADR 已回答

ADR-0009 已回答以下问题，并把后续实现拆成 PR-0026 起步：
...
下一步执行 PR-0026 / T-0028：基于 ADR-0009 实现 InMemory child session creation flow。该切片仍不实现 child result dispatcher、Parent reducer merge、capability delegation、真实 queue、database、server、provider 或 cloud。
