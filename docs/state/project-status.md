...
SessGraph 已完成初始规划 bootstrap、review follow-up、中文文档本地化修正，以及 PR-0002 P0 core data structures。初始产品讨论已经转换为仓库内持久的中文规划文件，后续 Codex、Claude Code 和人类会话共享同一事实源。

## 当前开发主线

**主线:** P0 durable session runtime core。

当前优先事项是 PR-0003：在已完成 P0 core data structures 后，实现 InMemory stores。PR-0003 的任务规格仍为 TBD，启动实现前需要先补齐任务规格。
...
补齐 PR-0003 的任务规格，然后实现 InMemory stores。不要在该 PR 中实现 Activation Runner、tools、wait/resume、providers、databases 或 server mode。
