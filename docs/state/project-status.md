...
> 状态: PR-0011 async job/timer ADR 已完成
> 最近更新: 2026-06-20

## 当前阶段

SessGraph 已完成初始规划 bootstrap、review follow-up、中文文档本地化修正、PR-0002 P0 core data structures、PR-0003 InMemory stores、PR-0004 最小 Activation Runner 循环、PR-0005 同步 tool execution flow、PR-0006 wait/resume user flow、PR-0007 P0 收尾审查、PR-0008 checkpoint recovery example/test、PR-0009 第二阶段规划，以及 PR-0011 async job/timer ADR。Owner 已明确 license 决策先延后，因此 PR-0010 package/release hygiene 延后。

## 当前开发主线

**主线:** 第二阶段 / P1 本地调度语义与开源可用性。

当前 PR 队列中的 P0 本地核心闭环已完成：data model、InMemory stores、Activation Runner、FakeModel、Checkpoint save/load recovery example、同步 tool execution、wait/resume 和确定性测试均已落地。第二阶段 / P1 已规划为 “本地调度语义与开源可用性”；package/release hygiene 已随 license 延后，async job/timer 公开语义已由 ADR-0005 固化，后续可按 PR-0012 / PR-0013 分别实现 InMemory timer flow 和 InMemory async job flow。
...
优先执行 PR-0012 InMemory timer flow；随后执行 PR-0013 InMemory async job flow。实现必须遵循 ADR-0005，不要在未立项前实现 providers、databases 或 server mode。
