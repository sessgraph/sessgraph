...
下一步执行 PR-0026 / T-0028：基于 ADR-0009 实现最小 InMemory child session creation flow，包括 `START_CHILD_SESSION` Decision、ChildSessionRecord/store、Child initial Signal、`child_session_started` Event、Checkpoint metadata 和 Safety/Auth gate。该切片不实现 `child_result` Signal dispatcher、Parent reducer merge、shared memory、capability delegation 或真实 queue/database/provider。
