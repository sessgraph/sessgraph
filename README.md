...
仓库已完成规划/bootstrap、P0 data model、InMemory stores、最小 Activation Runner、同步 tool execution、wait/resume user flow，以及 P0 收尾审查。当前目标不是快速堆实现，而是按小 PR 切片继续补齐已立项的 durable Session runtime core 缺口。
...
- Session Inbox、InMemory stores、FakeModel adapter、最小 Activation Runner、同步 Tool execution 和 wait/resume 已完成。
- 下一步候选是 checkpoint recovery example/test；其他后续能力应重新立项后再进入实现。
- 确定性测试和最小可运行 example。
