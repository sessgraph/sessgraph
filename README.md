...
仓库已完成规划/bootstrap、P0 data model、InMemory stores、最小 Activation Runner、同步 tool execution、wait/resume user flow、P0 收尾审查、checkpoint recovery example/test、第二阶段规划、package/release hygiene、P1 async job/timer、Memory + Context 本地确定性闭环、Safety/Auth ADR、InMemory capability policy gate、approval flow ADR、InMemory ApprovalRequest store、approval-required runner flow，以及 approval result runtime flow。当前目标不是快速堆实现，而是按小 PR 切片继续推进已立项的 durable Session runtime core 工作。
